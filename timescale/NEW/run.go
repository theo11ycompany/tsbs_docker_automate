...
func DockerBuildAndRun(dockerfile_path string, docker_image_name string, docker_config map[string]interface{}) string {
	...
	cmd = fmt.Sprintf("run -d --name timescaledb -m %s --cpus=%s -p 5432:5432 -e POSTGRES_PASSWORD=password timescale/timescaledb:latest-pg12", docker_config["memory"], docker_config["cpus"]) //[TODO] lets this port be configurable
	cmd_ctx = exec.Command("docker", strings.Split(cmd, " ")...)
	fmt.Println("\t[CMD]", cmd_ctx.String())
	out.Truncate(0)
	stderr.Truncate(0)
...
func main() {
	panicMessage := "failed to connect to `host=localhost user=postgres database=postgres`"
    ...
    defer func() {
        if r := recover(); r != nil {
            fmt.Println("[ERROR]", r)
            if r == panicMessage {
                fmt.Println("[INFO] Database connection failed. The code will continue running.")
            } else {
                // If it's a different panic, re-panic to halt the program
                panic(r)
            }
        }
    }()

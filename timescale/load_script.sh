...
cat /tmp/`echo "$target-data"`.gz | gunzip | ./tsbs_load_timescaledb load -`[ -z "$password" ] && echo "" || echo "--password $password"` --workers=$workers_value  > temp_data_insert_performance 

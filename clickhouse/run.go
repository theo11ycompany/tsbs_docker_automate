...
func plot_non_flat_maps(x_axis map[string][]float64, y_axis map[string][]float64) {
	...
	for x := range x_axis {
		for y := range y_axis {
			...
			if err := p.Save(8*vg.Inch, 8*vg.Inch, x+"_"+strings.Replace(y, "/", "_", -1)+".png"); err != nil {
				panic(err)
			}
		}
	}
...
func DockerBuildAndRun(dockerfile_path string, docker_image_name string, docker_config map[string]interface{}) string {
	...
	cmd = fmt.Sprintf("run -d -m %s --cpus=%s -p 9000:9000 %s:latest", docker_config["memory"], docker_config["cpus"], docker_image_name) //[TODO] lets this port be configurable
	cmd_ctx = exec.Command("docker", strings.Split(cmd, " ")...)
	fmt.Println("\t[CMD]", cmd_ctx.String())
	out.Truncate(0)
	stderr.Truncate(0)
...
func main() {
	...
	dat, err := os.ReadFile("./config_workers.json")
	if err != nil {
		println("err : ", err)
	}
